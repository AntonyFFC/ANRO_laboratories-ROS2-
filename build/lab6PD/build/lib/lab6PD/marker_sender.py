...
class MarkerSender(Node):
    def __init__(self):
        ...
        self.generate_marker_positions()
    ...
    def generate_marker_positions(self):
    ...
    def publish_markers(self):
        ...
        cube_position = np.matmul(np.linalg.inv(self.tac_matrix), cube_pose.T)
        paper_position = np.matmul(np.linalg.inv(self.tac_matrix), paper_pose.T)

        cube_point = Point(x=cube_position.item(0, 0), y=cube_position.item(0, 1), z=cube_position.item(0, 2))
        paper_point = Point(x=paper_position.item(0, 0), y=paper_position.item(0, 1), z=paper_position.item(0, 2))
        ...
        marker_pose_array.poses = [cube_marker_pose, paper_marker_pose]
...
def main(args=None):
    ...
    rclpy.spin(marker_sender)
    marker_sender.destroy_node()
    rclpy.shutdown()
